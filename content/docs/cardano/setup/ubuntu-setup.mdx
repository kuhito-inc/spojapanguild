...
<Callout type="info" title="`root`アカウント無効">
...
<Callout type="info" title="`root`アカウント有効">
...
> コマンド中の (SSHポートを入力してください) については[3. SSH設定変更](/docs/cardano/setup/ubuntu-setup#3-ssh)で設定したSSHポートを入力してください。(**)は不要です。
...
<Callout type="info" title="`Google Authenticator`を用いたSSH2段階設定（任意）">
...
## SSH 2FA（Google Authenticator）完全クリーンアップ [#sec-13]
