...
### デーモン再起動の自動化 [#automatic-service-restart]
...
[SSH鍵作成](/cardano/setup#sec-3)で生成した`authorized_keys`を対象サーバーの`$HOME/.ssh`ディレクトリにコピーします。
```mermaid
graph LR
...
<AccordionsYellow type="single">
  <Accordion id="ubuntu-setup-ssh-socket-not-active" title="activeではない場合">
...
<Callout type="warning" title="ターミナルソフト設定について">
...
> コマンド中の (SSHポートを入力してください) については[3. SSH設定変更](/cardano/setup/ubuntu-setup#sec-3)で設定したSSHポートを入力してください。  
(**)は不要です。
