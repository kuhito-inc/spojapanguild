...
プールIDが表示されていない場合は、[こちらの手順](/docs/cardano/setup/stake-pool-register#sec-4)を実行してください。  
...
[SPO BlockNotify設定](/docs/cardano/setup/blocknotify-setup)を導入することでブロックログDBに保存されるブロック生成ステータスを`LINE`/`Slack`/`Discord`/`Telegram`に通知することができます。  
