...
<Callout type="info" title="ブロックログについて">

このツールはウロボロスにおける自プールのブロック生成スケジュールを事前に取得するツールです。  
<span style={{color: "red"}}>ブロック生成スケジュールはセキュリティ上パブリックには公開されません。</span>
...
<Callout type="info" title="設定サーバー">
...
<Callout type="info" title="プログラム構成図">

<svg
  viewBox="0 0 1040 860"
  role="img"
  aria-label="cardano-node、CNCLI、Guild-DB、blocknotifyのプログラム構成図"
  style={{ width: '100%', height: 'auto' }}
>
  <defs>
    <marker id="program-diagram-arrow" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto">
      <path d="M0,0 L8,4 L0,8 Z" fill="currentColor" />
    </marker>
  </defs>

  <g fill="none" stroke="currentColor" strokeWidth="1.5" opacity="0.65" markerEnd="url(#program-diagram-arrow)">
    <path d="M470 90 C330 115 210 130 150 190" />
    <path d="M610 90 C730 120 800 140 835 190" />
    <path d="M150 250 V325" />
    <path d="M835 250 V325" />
    <path d="M835 250 C730 285 630 285 570 325" />
    <path d="M915 220 C1010 255 1030 330 940 500" />
    <path d="M570 385 C570 455 635 470 670 500" />
    <path d="M835 385 C835 455 780 470 735 500" />
    <path d="M720 560 V680" />
    <path d="M720 740 V795" />
    <path d="M940 540 V660" />
  </g>

  <g fill="var(--color-fd-muted)" stroke="currentColor" strokeWidth="1" opacity="0.22">
    <rect x="410" y="430" width="640" height="170" />
    <rect x="570" y="635" width="300" height="215" />
  </g>

  <g fill="var(--color-fd-card)" stroke="currentColor" strokeWidth="1">
    <rect x="430" y="40" width="180" height="60" />
    <rect x="70" y="190" width="160" height="60" />
    <rect x="755" y="190" width="160" height="60" />
    <rect x="40" y="325" width="220" height="60" />
    <rect x="470" y="325" width="200" height="60" />
    <rect x="735" y="325" width="200" height="60" />
    <rect x="640" y="500" width="160" height="60" />
    <rect x="860" y="500" width="160" height="60" />
    <rect x="635" y="680" width="170" height="60" />
    <rect x="860" y="660" width="160" height="60" />
    <rect x="655" y="795" width="130" height="50" />
  </g>

  <g fill="currentColor" fontSize="18" fontWeight="600" textAnchor="middle" dominantBaseline="middle">
    <text x="520" y="70">cardano-node</text>
    <text x="150" y="220">logファイル</text>
    <text x="835" y="220">cncli.service</text>
    <text x="150" y="355">logmonitor.service</text>
    <text x="570" y="355">leaderlog.service</text>
    <text x="835" y="355">validate.service</text>
    <text x="710" y="455">Guild-DB</text>
    <text x="720" y="530">blocklog.db</text>
    <text x="940" y="530">cncli.db</text>
    <text x="720" y="660">ステータス通知</text>
    <text x="720" y="710">blocknotify</text>
    <text x="940" y="690">blocks.sh</text>
    <text x="720" y="820">各アプリ</text>
  </g>
</svg>
...
<Callout type="info" title="CNCLIについて">
...
> `3.31.1`以上のバージョンがインストールされていれば問題ありません。

**SQLiteのタイムアウト設定**

データベースへの同時アクセスによるロックエラーを防ぐため、SQLiteのタイムアウトを設定します。
...
```yaml title="期待される表示例：" noCopy
-- Loading resources from /home/ユーザー名/.sqliterc
5000
```


## 4. 依存ファイルのダウンロード [#sec-4]
...
wget https://raw.githubusercontent.com/kuhito-inc/sjg-tools/main/blocks.sh -O ./blocks.sh 
wget https://raw.githubusercontent.com/kuhito-inc/sjg-tools/main/logMonitor.sh -q -O ./logMonitor.sh
```
...
## 5. サービスファイル作成・登録 [#sec-5]
```bash
mkdir -p $NODE_HOME/service
cd $NODE_HOME/service
```

<Tabs items={["cncli", "validate", "leaderlog", "logmonitor"]}>
<Tab value="cncli">
...
スクリプトへのパスを通し、エイリアスで起動出来るようにします。
```bash
...
> `cnclilog` の例：`cnode-cncli-sync`サービスのログが表示されます。  
> `validate` の例：`cnode-cncli-validate`サービスのログが表示されます。  
> `leaderlog` の例：`cnode-cncli-leaderlog`サービスのログが表示されます。  
> `logmonitor` の例：`cnode-logmonitor`サービスのログが表示されます。  

</Callout>


## 6. ブロックチェーンとDBを同期 [#sec-6]
...
<Callout type="info" title="ログ確認">

```yaml title="期待される表示例：" noCopy
INFO cncli::nodeclient::sync: block ******** of ********: 100.00% sync'd
```
> `100.00% sync'd`になるまでお待ちください。
> `Ctrl+c`で閉じます。
...
<Tabs items={["validate", "leaderlog", "logmonitor"]}>
<Tab value="validate">


<Callout type="info" title="ブロック検証確認">
...
<Callout type="info" title="スロットリーダー確認">
...
<Callout type="info" title="ログモニター確認">
...
```yaml title="期待される表示例：" noCopy
~~ LOG MONITOR STARTED ~~
monitoring /home/ユーザー名/cnode/logs/ノード名/node.json for traces
```
...
## 8. ブロック生成スケジュールと通知 [#sec-8]

[SPO BlockNotify設定](/cardano/setup/blocknotify-setup)を導入することでブロックログDBに保存されるブロック生成ステータスを`LINE`/`Slack`/`Discord`/`Telegram`に通知することができます。  

<Callout type="info" title="ブロック生成スケジュール取得のタイミング">
...
<Callout type="info" title="1エポックで1ブロック割り当てられる委任量の目安">
...
<Callout type="info" title="プール開設後のエポックの流れ">
...
<Callout type="info" title="制作クレジット">

このツールは海外ギルドオペレーター制作の[CNCLI By AndrewWestberg](https://github.com/cardano-community/cncli)、[Guild LiveView](https://cardano-community.github.io/guild-operators/#/Scripts/gliveview)、[BLOCK LOG for CNTools](https://cardano-community.github.io/guild-operators/#/Scripts/cntools)を組み合わせたツールとなっております。  
カスタマイズするにあたり、開発者の[AHLNET(AHL)](https://twitter.com/olaahlman)にご協力頂きました。  
ありがとうございます。
