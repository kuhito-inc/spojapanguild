...
これらの秘密鍵をオンライン環境（BP／リレーノード等）に置くと、ハッキングやマルウェア感染などにより、**資金やプール運営権限の奪取につながる重大なリスク**があります。  
**秘密鍵の管理と署名処理は、必ずエアギャップマシン上で行ってください。**
...
## エアギャップマシンの構築手順 [#sec-2]

<Callout type="error" title="インストール中はネットワークに接続しない">

Ubuntu をインストールする対象のエアギャップマシンは、**インストール作業中も LAN／Wi‑Fi などに接続しないでください。**
