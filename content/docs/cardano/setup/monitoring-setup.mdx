...
プロメテウスはターゲットに指定したメトリックHTTPエンドポイントをスクレイピングし、情報を収集する監視ツールです。[オフィシャルドキュメントはこちら](https://prometheus.io/docs/introduction/overview/)  
グラファナは収集されたデータを視覚的に表示させるダッシュボードツールです。

## 1. インストール [#sec-1]

`prometheus` と `prometheus node exporter` をインストールします。  
この手順では、**Grafana稼働サーバー（リレーサーバー1）** で`prometheus`と`Grafana`本体を稼働させ、各サーバーの情報を取得します。
...
- `xxx.xxx.xxx.xxx`：BPのパブリックIPアドレス
- `bb.xxx.xxx.xxx`：リレー2のパブリックIPアドレス
...
以下3つのサービスが緑色の `active (running)` になっていることを確認してください。
