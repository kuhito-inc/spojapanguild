...
プール登録後にメタ情報、誓約、固定費、変動費、リレー情報を変更する場合は、[運用証明書(pool.cert)の更新](/docs/cardano/operation/cert-update)の変更手順を実施してください。

</Callout>



<Callout type="warn" title="プール登録料">
...
　<span style={{color: "red"}}>ユーザー名を最大13文字以内で作成してください</span>

2.Repositoriesタブをクリックし、右上の**New** をクリックします。 

3.リポジトリ作成

* **Repository name**に任意のリポジトリ名<span style={{color: "red"}}>(最大13文字以内)</span>を入力
* Publicを選択
...
11．**BP**でjsonファイルをダウンロードし、ハッシュ値を計算。  

<Callout type="error" title="URLを書き換えてから実行して下さい">
...
BPにある`vrf.vkey`と`poolMetaDataHash.txt` をエアギャップのcnodeディレクトリにコピーします。
```mermaid
graph LR
...
<Callout type="info" title="リレー(--pool-relay-ipv4)の記述について▼">
...
[xxx.xxx.xxx]をリレーのパブリックIP(静的)アドレスへ書き換え
```bash
...
<Callout type="info" title="あと一息です！">
