...
<AccordionsBlue type="single">
  <Accordion id="r-login-recommended-settings" title="R-Login 推奨設定">
...
<Callout type="info" title="Terminal（macOS 標準ターミナル）">
...
<Callout type="info" title="Termius（Windows / macOS / Linux / iOS / Android）">
...
<AccordionsBlue type="single">
  <Accordion id="setup-index-win-openssh-not-found" title="表示されない場合">

1. `Win + R` を押下し、`cmd`と入力後、`Ctrl + Shift + Enter`→「`はい`」を選択
```cmd
powershell -Command "Add-WindowsCapability -Online -Name OpenSSH.Client~~~~0.0.1.0"
```
2. 追加後、`where ssh`を実行し、PATHが表示されていることを確認。  
> OpenSSH の追加のみ管理者権限が必要です。  
> 以降は通常モードの `cmd` で実行してください。
