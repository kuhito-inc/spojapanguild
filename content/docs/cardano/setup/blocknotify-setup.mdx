...
| `line_notify_token`     |[LINE設定](/docs/cardano/setup/blocknotify-setup#__tabbed_1_1)で発行したチャンネルアクセストークン | Line通知用のトークンを入力 |
| `line_user_id`     |[LINE設定](/docs/cardano/setup/blocknotify-setup#__tabbed_1_1)で発行したユーザーID | Line通知用のユーザーIDを入力 |
| `discord_webhook_url`   |[Discord設定の(7)](/docs/cardano/setup/blocknotify-setup#__tabbed_1_2)で発行したウェブフックURL| DiscordウェブフックURLを入力 |
| `slack_webhook_url`   |[Slack設定の(4)](/docs/cardano/setup/blocknotify-setup#__tabbed_1_4)で発行したWebhook URL| SlackウェブフックURLを入力 |
| `telegram_token`   |[Telegram設定の(5)](/docs/cardano/setup/blocknotify-setup#__tabbed_1_3)で発行したAPIトークン | Telegram APIトークンを入力 |
| `telegram_id`   |[Telegram設定の(9)](/docs/cardano/setup/blocknotify-setup#__tabbed_1_3)で表示されたChat id| Telegram ChatIDを入力 |
...
<span style={{color: "red"}}>新しいAPIはLINE公式アカウントを通じて配信されるため、設定を間違えるとセキュリティ漏洩のリスクがあります。複数人で運用する場合はご注意ください</span>  
<br />
上記の点を踏まえ、他の通知システム(Discord/Telegram/Slack)へ移行するか新しい[LINE通知の設定](/docs/cardano/setup/blocknotify-setup#__tabbed_1_1)を実施してください。（流れで11-3まで実施しないでください）  
