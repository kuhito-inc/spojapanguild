...
依存関係をインストールします。
```bash
sudo apt install bc curl htop nano needrestart protobuf-compiler rsync ufw zstd automake build-essential pkg-config libffi-dev libgmp-dev libssl-dev libncurses-dev libsystemd-dev zlib1g-dev make g++ tmux git jq wget -4 libtool autoconf liblmdb-dev liburing-dev libsnappy-dev -y
```
...
ファイルをダウンロードします。
```bash
wget -4 --no-use-server-timestamps https://download.spojapanguild.net/node_config/11.1.2/${NODE_CONFIG}-byron-genesis.json -O ${NODE_CONFIG}-byron-genesis.json
wget -4 --no-use-server-timestamps https://download.spojapanguild.net/node_config/11.1.2/${NODE_CONFIG}-shelley-genesis.json -O ${NODE_CONFIG}-shelley-genesis.json
wget -4 --no-use-server-timestamps https://download.spojapanguild.net/node_config/11.1.2/${NODE_CONFIG}-alonzo-genesis.json -O ${NODE_CONFIG}-alonzo-genesis.json
wget -4 --no-use-server-timestamps https://download.spojapanguild.net/node_config/11.1.2/${NODE_CONFIG}-conway-genesis.json -O ${NODE_CONFIG}-conway-genesis.json
wget -4 --no-use-server-timestamps https://download.spojapanguild.net/node_config/11.1.2/${NODE_CONFIG}-config.json -O ${NODE_CONFIG}-config.json
wget -4 --no-use-server-timestamps https://download.spojapanguild.net/node_config/11.1.2/${NODE_CONFIG}-topology.json -O ${NODE_CONFIG}-topology.json
wget -4 --no-use-server-timestamps https://download.spojapanguild.net/node_config/11.1.2/${NODE_CONFIG}-checkpoints.json -O ${NODE_CONFIG}-checkpoints.json
```
...
```bash
wget -4 --no-use-server-timestamps https://download.spojapanguild.net/node_config/11.1.2/${NODE_CONFIG}-peer-snapshot.json -O ${NODE_CONFIG}-peer-snapshot.json
```
...
```bash
wget -4 --no-use-server-timestamps https://download.spojapanguild.net/node_config/11.1.2/${NODE_CONFIG}-tracer-config.json -O ${NODE_CONFIG}-tracer-config.json
```
...
### トポロジーファイルの設定 [#topology-generator]

Genesis Mode 構成のトポロジーを生成します。接続先のポートは、各ノードの起動スクリプトで設定した値に合わせてください。
リレー用では、ダウンロードした Peer Snapshot ファイルの絶対パスを指定します。BP用では Peer Snapshot を使用しません。

<TopologyGenerator />

生成したファイル作成コマンドを対象ノードで実行するか、ダウンロードした JSON を `${NODE_HOME}/${NODE_CONFIG}-topology.json` に配置してください。
起動前に構文を確認します。
...
リレーサーバーで[バイナリファイルをコピー](/cardano/operation/node-update#sec-30)を実施します。
