...
| :----------: | :----------: | :----------: | :----------: | :----------: |
| 11.0.1 | 11.0.0.0 | 9.6.7 | 3.12.1.0 | 6.7.0 |
...
echo export CARDANO_NODE_NETWORK_ID=764824073 >> $HOME/.bashrc
```
...
ファイルをダウンロードします。
```bash
wget --no-use-server-timestamps https://spojapanguild.net/node_config/11.0.1/${NODE_CONFIG}-byron-genesis.json -O ${NODE_CONFIG}-byron-genesis.json
wget --no-use-server-timestamps https://spojapanguild.net/node_config/11.0.1/${NODE_CONFIG}-shelley-genesis.json -O ${NODE_CONFIG}-shelley-genesis.json
wget --no-use-server-timestamps https://spojapanguild.net/node_config/11.0.1/${NODE_CONFIG}-alonzo-genesis.json -O ${NODE_CONFIG}-alonzo-genesis.json
wget --no-use-server-timestamps https://spojapanguild.net/node_config/11.0.1/${NODE_CONFIG}-conway-genesis.json -O ${NODE_CONFIG}-conway-genesis.json
wget --no-use-server-timestamps https://spojapanguild.net/node_config/11.0.1/${NODE_CONFIG}-config.json -O ${NODE_CONFIG}-config.json
wget --no-use-server-timestamps https://spojapanguild.net/node_config/11.0.1/${NODE_CONFIG}-topology.json -O ${NODE_CONFIG}-topology.json
wget --no-use-server-timestamps https://spojapanguild.net/node_config/11.0.1/${NODE_CONFIG}-checkpoints.json -O ${NODE_CONFIG}-checkpoints.json
```
...
```bash
wget --no-use-server-timestamps https://spojapanguild.net/node_config/11.1.0/${NODE_CONFIG}-tracer-config.json -O ${NODE_CONFIG}-tracer-config.json
```
...
- `networkMagic`：使用するネットワークのNetwork Magic
- `network.contents`：`${NODE_HOME}/tracer/socket`
- `network.tag`：`AcceptAt`
- `logging[0].logRoot`：`${NODE_HOME}/logs`
- `logging[0].logMode`：`FileMode`
- `logging[0].logFormat`：`ForMachine`
- `hasEKG`：`null`
- `hasPrometheus.epHost`：`0.0.0.0`
- `hasPrometheus.epPort`：`12808`
...
リレーサーバーで[バイナリファイルをコピー](/cardano/operation/node-update#sec-15)を実施します。

### 環境変数を設定 [#sec-22]
```bash
echo 'export NODE_HOME="$HOME/cnode"' >> ~/.bashrc
echo 'export NODE_NETWORK="--mainnet"' >> ~/.bashrc
echo 'export CARDANO_NODE_NETWORK_ID=764824073' >> ~/.bashrc
