...
ファイル確認
```
for f in "${NODE_CONFIG}"-{byron-genesis,shelley-genesis,alonzo-genesis,conway-genesis,config,topology,checkpoints}.json; do jq empty "$f" >/dev/null 2>&1 && echo "OK  $f" || echo "NG  $f"; done
```
