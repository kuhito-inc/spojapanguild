...
このガイドは ノードバージョン`11.1.1`に対応しています。  
...
| :----------: | :----------: | :----------: | :----------: | :----------: | :----------: |
| ubuntu 24.04.* | 11.1.1 | 11.2.3.0 | 9.6.7 | 3.12.1.0 | 6.7.0 |

**■アップデートパターンDB再構築有無**

| バージョン | DB再構築有無 | 設定ファイル更新有無 | トポロジーファイル更新有無 |
| :----------: | :----------: | :----------: | :----------: |
| 11.0.1 → 11.1.1 | なし | 更新あり | 更新あり |
...
■検証環境
PreProd-Testnet / cardano-node 11.1.1 / cardano-cli 11.2.3.0 / SJG-TOOL 4.2.0  
...
wget https://github.com/IntersectMBO/cardano-node/releases/download/11.1.1/cardano-node-11.1.1-linux-amd64.tar.gz
```

解凍
```bash
tar zxvf cardano-node-11.1.1-linux-amd64.tar.gz ./bin/cardano-node ./bin/cardano-cli ./bin/snapshot-converter ./bin/cardano-tracer
```
...
Cardano Nodeの状態を確認します。

```bash
systemctl status --no-pager cardano-node.service
```

<AccordionsBlue type="single">

  <Accordion id="restart-dmq-node-after-systemd-update" title="DMQ Nodeを導入している場合">

DMQ Nodeを導入している場合は、DMQ Nodeを再起動します。

```bash
sudo systemctl restart dmq-node.service
```

DMQ Nodeの状態を確認します。

```bash
systemctl status --no-pager dmq-node.service
```
...
## 5. Grafana Dashboard設定 [#sec-22]

### Dashboardのインポート [#sec-23]
