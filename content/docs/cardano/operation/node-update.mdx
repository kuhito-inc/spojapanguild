...
- OSバージョンアップを完了していること  
[Ubuntu22.04から24.04へ移行](/cardano/operation/ubuntu24-migration)
...
wget -q https://github.com/IntersectMBO/cardano-node/releases/download/11.0.1/cardano-node-11.0.1-linux-amd64.tar.gz
```

解凍
```bash
tar zxvf cardano-node-11.0.1-linux-amd64.tar.gz ./bin/cardano-node ./bin/cardano-cli ./bin/snapshot-converter
```
...
<AccordionsYellow type="single">
  <Accordion id="node-update-from-1031-below-extra" title="10.3.1 未満のバージョンから更新する場合は、こちらを実施">
...
解凍
```bash
tar zxvf cardano-node-11.0.1-linux-amd64.tar.gz ./bin/snapshot-converter
```
...
```bash
cd $NODE_HOME
wget -q https://spojapanguild.net/node_config/11.0.1/${NODE_CONFIG}-config.json -O ${NODE_CONFIG}-config.json

wget -q https://spojapanguild.net/node_config/11.0.1/${NODE_CONFIG}-checkpoints.json -O ${NODE_CONFIG}-checkpoints.json
```


<AccordionsYellow type="single">
  <Accordion id="node-update-from-1014-extra" title="10.7.1 未満のバージョンから更新する場合は、こちらを実施">
...
<AccordionsBlue type="single">
  <Accordion id="node-update-backup-old-db" title="旧dbをバックアップしたい方はこちら">
...
### Grafanaアラートの対象メトリクス変更 [#sec-14]

> すでに対応済みの場合は、スキップしてください。
