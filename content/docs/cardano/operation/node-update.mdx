...
<Callout type="error" title="主な変更点と新機能および検証結果">
...
<Callout type="error" title="各アプリのバージョン(戻り値)が異なる場合">


<Callout type="error" title="cabal 3.8.1.0以下の場合">
...
<Callout type="error" title="GHC 9.6.7以外の場合 (9.6.7より新しい場合を含む)">
...
<Callout type="error" title="libsodiumコミット値が違う場合">
...
<Callout type="error" title="secp256k1コミット値が違うまたは戻り値が無い場合">
...
<Callout type="error" title="Blst 0.3.13以下または「No such file or directory」の場合">
...
<Callout type="error" title="cncli v6.6.0以下だった場合(クリックして開く)">
...
<Callout type="warn" title="10.3.1以下からアップデートする場合はこちらも実施(クリックして開く)">
...
<Callout type="warn" title="10.1.4~からアップデートする場合はこちらも実施(クリックして開く)">
...
<Callout type="error" title="ブロック生成ステータス通知またはSPO Block Notifyを未導入(更新)していない方">


<Tabs items={["未導入の方で今後も導入予定が無い方", "新規導入または更新する方"]}>
<Tab value="未導入の方で今後も導入予定が無い方">

[5. サービスファイル作成・登録](/docs/cardano/setup/blocklog-setup#5)を再実行してください。（エイリアス設定は不要です）

</Tab>
<Tab value="新規導入または更新する方">

[SPOBlockNotify移行手順](/docs/cardano/setup/blocknotify-setup)から新規導入または更新を実施してください。
...
<Callout type="error" title="cnclilogで`Missing eta_v for block xxxxxx` エラーが出る場合の対処法">
