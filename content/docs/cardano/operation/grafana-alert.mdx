...
- grafana version 13.2.2以上
- Grafana [SJG最新ダッシュボード](/cardano/setup/monitoring-setup#sec-6)適用済み

### Grafanaバージョン確認 [#sec-2]
...
<Callout type="info" title="通知先アプリの設定">

アラートの通知先はLINE/Discord/Telegram/Slackを複数指定することが可能です。  
ブロック生成ステータス通知の[2. 通知アプリの設定](/cardano/setup/blocknotify-setup#sec-2)で設定した手順と同様に、通知先名などを変えてトークンを発行してください。  
...
- 左上部の３本の横線マークを選択し、「`Alerting`」→「`Notification configuration`」→「`Templates`」タブを選択し、右側にある「`+ New notification template`」を選択
...
> ここではDiscordを選択しています。
- [2. 通知アプリの設定](/cardano/setup/blocknotify-setup#sec-2)で取得した通知アプリごとのトークンIDやWebhookURLを入力
...
<Callout type="info" title="通知先ごとのタグ入力欄表記について">
...
<Callout type="info" title="複数通知先の設定について">
...
- 左上部の３本の横線マークを選択し、「`Alerting`」→「`Alert rules`」→画面中央の「`New alert rule`」の順に選択します。
...
```
- 「`Save`」を選択
![](/images/grafana-alert/grafana-alert-settings_13.png)
...
上記で作成したルールをコピーします。
「`Alert rules`」から`SJG`→`ノード監視`→`Relay1-スロット監視`→`Edit`→`Duplicate`を選択します。
![](/images/grafana-alert/grafana-alert-settings_14.png)
...
## 6. 通知内容URLカスタマイズ [#sec-14]

<Callout type="info" title="注意">

- 事前に[Grafanaセキュリティ強化](/cardano/operation/grafana-security)を実施してください。
...
`xxxx.bbb.com`を[Grafanaセキュリティ強化](/cardano/operation/grafana-security#sec-2)で取得したドメイン(サブドメイン)に置き換えて実行  
`https://`は不要
```bash
domain=xxxx.bbb.com
```
