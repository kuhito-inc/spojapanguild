...
* Grafana [SJG最新ダッシュボード](/docs/cardano/setup/monitoring-setup#3-grafana)適用済み

### 1-1. Grafanaバージョン確認 [#sec-2]

```bash
grafana-cli --version
```

### 1-2. システムパッケージの更新 [#sec-3]
```bash
...
ブロック生成ステータス通知の[2. 通知アプリの設定](/docs/cardano/setup/blocknotify-setup#2)で設定した手順と同様に、通知先名などを変えてトークンを発行してください。  
...
> ここではDiscordを選択しています。
* [2. 通知アプリの設定](/docs/cardano/setup/blocknotify-setup#2)で取得した通知アプリごとのトークンIDやWebhookURLを入力
...
* 事前に[Grafanaセキュリティ強化](/docs/cardano/operation/grafana-security)を実施してください。
...
`xxxx.bbb.com`を[Grafanaセキュリティ強化](/docs/cardano/operation/grafana-security#1)で取得したドメイン(サブドメイン)に置き換えて実行  
`https://`は不要
```
domain=xxxx.bbb.com
```
