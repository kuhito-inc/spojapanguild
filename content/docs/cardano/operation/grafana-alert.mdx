...
- Grafana [SJG最新ダッシュボード](/docs/cardano/setup/monitoring-setup#sec-4)適用済み
...
ブロック生成ステータス通知の[2. 通知アプリの設定](/docs/cardano/setup/blocknotify-setup#sec-2)で設定した手順と同様に、通知先名などを変えてトークンを発行してください。  
...
> ここではDiscordを選択しています。
- [2. 通知アプリの設定](/docs/cardano/setup/blocknotify-setup#sec-2)で取得した通知アプリごとのトークンIDやWebhookURLを入力
...
`xxxx.bbb.com`を[Grafanaセキュリティ強化](/docs/cardano/operation/grafana-security#sec-2)で取得したドメイン(サブドメイン)に置き換えて実行  
`https://`は不要
```bash
domain=xxxx.bbb.com
```
