...
<Callout type="info" title="バトル">

<Callout type="info" title="通称：スロットバトル（SlotBattle / Stolen）">
...
<Callout type="info" title="通称：ハイトバトル（HeightBattle / Ghosted）">
...
Ouroboros Praosでは、VRFキーを使って 各スロットでプールがスロットリーダーに選ばれるかどうかをランダムかつ検証可能な方法で判定します。  
...
## 各種鍵ファイルの役割と保管場所 [#sec-6]


### 証明書および鍵ファイルの詳細 [#sec-7]
🔒・・・ロック必須・再作成不可・紛失不可  
🔄・・・更新時書き換え・再作成可能  
🔴・・・BP起動で使用  
🔷・・・ブロックログで使用

<Callout type="warn" title="鍵ファイルバックアップについて">
...
| **myCalidusKey.vkey**       | Calidus公開鍵  | エアギャップ ／ USB | 🔒 |
| **myCalidusKey.skey**       | Calidus秘密鍵 | **BP** ／ エアギャップ ／ USB | 🔒 |
...
| **Calidus-MnemonicsKey.json**    | Calidusニーモニックファイル | エアギャップ ／ USB | 🔄 |
| **myCalidusRegistrationMetadata.json**    | Calidus登録メタデータファイル | エアギャップ ／ USB | 🔄 |

> Calidus関連ファイル（`myCalidusKey.vkey`、`myCalidusKey.skey`、`Calidus-MnemonicsKey.json`、`myCalidusRegistrationMetadata.json`）は、[Calidusキー生成](/docs/cardano/operation/calidus-key-generation)を実施した場合に作成されるファイルです。  
...
| **poolMetaData.json**      | プールメタデータファイル  | BP | 🔄 |
...
## 運用タスクチェックリスト [#sec-10]
<Callout type="info" title="各作業">

<Callout type="info" title="日次作業">
...
<Callout type="info" title="エポック毎作業">
...
<Callout type="info" title="3か月毎作業">

- [KESの更新](/docs/cardano/operation/kes-update)（KES鍵の再生成および証明書更新）

</Callout>


<Callout type="info" title="不定期作業">
