...
ドキュメントとして記載しておきますが、実運用では、半自動更新が可能な[SJGツール](/cardano/operation/sjg-tool-setup)の利用を推奨します。
...
8. **エアギャップ**：`node.cert`を生成 ([項目7](/cardano/operation/kes-update#sec-7)で算出した`KesPeriod`を使うこと)   
...
```
> 上記のコマンドをそのままコピーしてターミナルに入力してください。  
> コマンド実行後に、数字入力モードになりますので[項目5](/cardano/operation/kes-update#sec-5)で確認した更新カウンター番号を入力します。
...
```
> 上記のコマンドをそのままコピーしてターミナルに入力してください。  
> コマンド実行後に、数字入力モードになりますので[項目7](/cardano/operation/kes-update#sec-7)で算出した`startKesPeriod`の数字を入力します。  
...
確認のため、[SJGツール](/cardano/operation/sjg-tool-setup)を実行します。
