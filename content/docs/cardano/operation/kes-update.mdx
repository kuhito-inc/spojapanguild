...
ドキュメントとして記載しておきますが、<span style={{color: "red"}}>**実運用では、半自動更新が可能な[SJG TOOL](/docs/cardano/operation/sjg-tool-setup)の利用を推奨します。**</span>
...
<Callout type="info" title="KES更新の流れ">
...
8. **エアギャップ**：`node.cert`を生成 ([項目7](/docs/cardano/operation/kes-update#7-bpkesperiod)で算出した`KesPeriod`を使うこと)   
...
## 1. KES更新タイミングの確認 [#sec-1]
<Tabs items={["ブロックプロデューサー"]}>
<Tab value="ブロックプロデューサー">
...
## 5. オンチェーンカウンター取得 [#sec-5]
<Tabs items={["ブロックプロデューサー"]}>
<Tab value="ブロックプロデューサー">

```bash
kesperiodinfo=$(cardano-cli latest query kes-period-info $NODE_NETWORK --op-cert-file $NODE_HOME/node.cert --out-file kesperiod.json)
...
```
> 上記のコマンドをそのままコピーしてターミナルに入力してください。  
> コマンド実行後に、数字入力モードになりますので[項目5](/docs/cardano/operation/kes-update#5)で確認した更新カウンター番号を入力します。
...
```
> 上記のコマンドをそのままコピーしてターミナルに入力してください。  
> コマンド実行後に、数字入力モードになりますので[項目7](/docs/cardano/operation/kes-update#7-bpkesperiod)で算出した`startKesPeriod`の数字を入力します。  
...
<Callout type="error" title="10分以上経ってもノードが同期を再開しない場合はこちら">
...
## 12. チェックプログラム実行 [#sec-12]
確認のため、**[SJG TOOL](/docs/cardano/operation/sjg-tool-setup)**を実行します。
