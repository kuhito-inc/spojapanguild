...
| Cardano Node | Mithril Signer | Squid |
| :----------- | :------------- | :---- |
| 11.1.2       | 1.1.9          | 7.7   |
...
ビルド済みバイナリのダウンロード
```bash
curl --proto '=https' --tlsv1.2 -sSf https://raw.githubusercontent.com/IntersectMBO/mithril/refs/heads/main/mithril-install.sh | sh -s -- -c mithril-signer -d 2630.1-hotfix -p $HOME/git/mithril
```
...
パラメータ値算出
```bash
era_params=$(jq -nc --arg address $(wget -4 -q -O - https://raw.githubusercontent.com/IntersectMBO/mithril/main/mithril-infra/configuration/release-mainnet/era.addr) --arg verification_key $(wget -4 -q -O - https://raw.githubusercontent.com/IntersectMBO/mithril/main/mithril-infra/configuration/release-mainnet/era.vkey) '{"address": $address, "verification_key": $verification_key}')
```
...
```bash
cd $HOME
wget -4 https://github.com/squid-cache/squid/releases/download/SQUID_7_7/squid-7.7.tar.gz
```

```bash
tar xzf squid-7.7.tar.gz
cd squid-7.7
```
...
```bash
cd $HOME/mithril
wget -4 https://mithril.network/doc/scripts/verify_signer_registration.sh
```
```bash
...
`Squid Cache: Version 7.7` の戻り値がある場合、以降実施不要です。
