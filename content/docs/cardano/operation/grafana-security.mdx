...
以下の手順は、[監視ツール設定](/cardano/setup/monitoring-setup)で設定したGrafana導入サーバーで実施してください。
...
`xxxx.bbb.com`を[1. ドメイン(サブドメイン)の取得](/cardano/operation/grafana-security#sec-2)で取得したドメイン(サブドメイン)に置き換えて実行します。
...
```
> 戻り値が設定したドメインであること。  
> そうでなければ、変数domainに[1. ドメイン(サブドメイン)の取得](/cardano/operation/grafana-security#sec-2)で取得したドメイン(サブドメイン)を代入してください。
