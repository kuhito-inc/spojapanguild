...
**payment.addrへ送金する方法**は[こちら](#sec-2)

**任意のアドレスへ送金する方法は**[こちら](#sec-3)
...
報酬の移動先となる`payment.addr`を設定します。  
`payment.addr`には取引手数料を支払うための残高が必要です。
...
**tx.signed** を **BP**のcnodeディレクトリにコピーします。
