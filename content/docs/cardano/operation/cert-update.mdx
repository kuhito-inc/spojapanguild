...
オンラインファイルハッシュ値確認
> 公開したメタデータファイルをダウンロードし、ハッシュ値を確認します。
...
```bash
cd $NODE_HOME
wget -4 -O onlineMetaData.json "$metadata_url"
```
...
BPの`poolMetaDataHash.txt`と`metadata-url.txt` をエアギャップのcnodeディレクトリにコピーします。
```mermaid
graph LR
    A[BP] -->| poolMetaDataHash.txt / metadata-url.txt | B[エアギャップ];
```
...
このタブではプールメタデータの変更はありません。  
前回登録時に使用した `metadata-url.txt` がエアギャップに存在することを確認したうえで、**登録証明書トランザクションを作成する**以降の手順へ進んでください。  
`metadata-url.txt` が存在しない場合は、以下のコマンドで作成してください。

エアギャップで実行します。  

存在確認
```bash
ls -l metadata-url.txt
```
...
cardano-cli latest transaction submit \
  ...
  $NODE_NETWORK
```
> `Transaction successfully submitted`と表示されれば成功
