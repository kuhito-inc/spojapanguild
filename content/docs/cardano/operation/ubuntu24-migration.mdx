...
[作業対象サーバログイン](/cardano/operation/ubuntu24-migration#sec-5)〜[Ubuntuバージョン確認](/cardano/operation/ubuntu24-migration#sec-10)は通常のSSH接続で作業してください。

</Callout>


<AccordionsBlue type="single">
  <Accordion id="ubuntu24-migration-id-rsa" title="id_rsaの場合">
...
<AccordionsYellow type="single">
  <Accordion id="ubuntu24-migration-python-alternatives" title="update-alternativesでPythonを管理していた場合">
...
<AccordionsYellow type="single">
  <Accordion id="ubuntu24-migration-deadsnakes-extra-python" title="サードパーティリポジトリからpython3.10以外を導入していた場合">
...
<AccordionsBlue type="single">
  <Accordion id="ubuntu24-migration-apt-has-upgradable" title="ある場合">
...
### リレーサーバーにGrafanaを搭載している場合の対応 [#sec-11]

<AccordionsYellow type="single">
  <Accordion id="ubuntu24-migration-grafana-disable-before-upgrade" title="Grafana搭載サーバー">
...
## 3. アップグレード確認(SSH接続) [#sec-16]

<Callout type="info" title="接続方法">


[3. アップグレード確認(SSH接続)](/cardano/operation/ubuntu24-migration#sec-16)〜[5. sysctlの設定](/cardano/operation/ubuntu24-migration#sec-22)はSSH接続で作業してください。

</Callout>


### 現在のUbuntuバージョン確認 [#sec-17]
...
### リレーサーバーにGrafanaを搭載している場合の対応 [#sec-20]

<AccordionsYellow type="single">
  <Accordion id="ubuntu24-migration-grafana-restore-after-upgrade" title="Grafana搭載サーバー">
...
```bash
wget -4 -q -O - https://apt.grafana.com/gpg.key \
| gpg --dearmor \
...
ノード同期状況の確認
```bash
sudo journalctl -q -u cardano-node -f -o cat | jq -cC --unbuffered .
```
...
<AccordionsYellow type="single">
  <Accordion id="ubuntu24-migration-python-dep-error" title="Python依存関係エラーが発生した場合">
...
現在の最新バージョンはv2.5.*なのでそれ以下の方は、[4. アップデート手順](/cardano/setup/blocknotify-setup#sec-4)を実施して更新してください。
...
<span style={{color: "red"}}>ソースコードからビルドした`cardano-node`/`cardano-cli`を使用していた場合、もしくはノードアップデートする場合は、以下の手順で現行ノードへ更新してください。</span>
<Callout type="info" title="ヒント">

[ノードセットアップ](/cardano/setup/node-setup)の[1. 依存関係インストール](/cardano/setup/node-setup#sec-1)から実施してください。Tracer設定に加え、関連ツール・サービスも確認します。

Grafanaを使用している場合は、[監視環境の構築](/cardano/setup/monitoring-setup)の[5. Grafanaダッシュボードの設定](/cardano/setup/monitoring-setup#sec-6)まで実施してください。

エアギャップの`cardano-cli`も更新する場合は、[エアギャップのセットアップ](/cardano/setup/node-setup#sec-20)も実施してください。
