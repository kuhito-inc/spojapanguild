...
- 旧BPの**サーバー本体（OS）**は、設定ファイルの確認などに使用するため、[旧BPシャットダウン](/cardano/operation/bp-migration#sec-21)まで起動しておいてください。`cardano-node`サービスは[旧BPノード停止](/cardano/operation/bp-migration#sec-6)で停止し、新BPへ切り替えた後は起動しないでください。
...
1. サーバ独自機能に留意してください。※さくらのパケットフィルタや、AWSのFW設定などのサーバー独自の機能に気を付けてください。
...
### Ubuntu初期設定 [#sec-2]
新サーバーで[Ubuntu初期設定](/cardano/setup/ubuntu-setup)を実施します。  


### ノードセットアップ [#sec-3]
1. [1. 依存関係インストール](/cardano/setup/node-setup#sec-1) 〜
[8. gLiveViewのインストール](/cardano/setup/node-setup#sec-19)まで実施します。
2. [2. トポロジーファイル設定変更](/cardano/setup/relay-bp-setup#sec-5)の`BP`の手順を実施します。
...
新規BPサーバーで[5. BPノードとして再起動](/cardano/setup/bp-key-setup#sec-5)を実行し、「`startBlockProducingNode.sh`」を生成してください。
...
- [ブロックログ設定](/cardano/setup/blocklog-setup)


### SJGツール導入 [#sec-14]

- [SJGツール導入設定](/cardano/operation/sjg-tool-setup)
...
1. 新BPで[1. 依存プログラムのインストール](/cardano/setup/blocknotify-setup#sec-1)のみを実施
2. 旧BPで設定ファイルを確認
...
3. 新BPで[3. 通知プログラムの設定](/cardano/setup/blocknotify-setup#sec-3)を実施します。
...
[SPO BlockNotify設定](/cardano/setup/blocknotify-setup)から導入してください。
...
### 新BP`node exporter`インストール [#sec-18]
...
```bash
sudo ufw allow from <Grafana稼働サーバーのIP> to any port 12808
```
```bash
...
### Grafanaサーバー`prometheus.yml`の修正 [#sec-19]
