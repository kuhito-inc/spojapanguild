...
- 旧BPは[6-1. 旧BPシャットダウン](/cardano/operation/bp-migration#sec-21)まで、**起動状態** にしておいてください。
...
新規BPサーバーで[5. BPノードとして再起動](/cardano/setup/bp-key-setup#5-bp)を実行し、「`startBlockProducingNode.sh`」を生成してください。
...
1. 新BPで[1. 依存プログラムのインストール](/cardano/setup/blocknotify-setup#1)のみを実施
2. 旧BPで設定ファイルを確認
...
3. 新BPで[3. 通知プログラムの設定](/cardano/setup/blocknotify-setup#3)を実施します。
