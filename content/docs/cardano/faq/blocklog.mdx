---
title: "よくある質問：ブロックログ関連"
---

## Q1. ブロック生成に失敗しました。原因は何でしょうか？ [#sec-1]

<Callout type="info" title="A. ステータスを確認します">

<Tabs items={["Stolenの場合", "Ghostedの場合", "Missedの場合", "Invalidの場合"]}>
<Tab value="Stolenの場合">
...
<Tab value="Ghostedの場合">

**通称：ハイトバトル（HeightBattle / Ghosted）**

Ghostedは、主に次の2つの理由で発生します。  

<Callout type="info" title="**原因1：VRF値による判定(運要素)**">
...
<Callout type="info" title="**原因2：サーバー時間のズレ**">
...
<Callout type="info" title="**slotsMissedとは？**">
...
<Callout type="info" title="**該当スロットのログが表示される場合**">
...
<Callout type="info" title="**該当スロットのログが表示されない場合**">

ノードがシャットダウンしていたか、`slotsMissed`が発生していた可能性があります。  
リレー1台構成の場合、そのリレーが落ちるとBPは、ブロックを生成できません。  
リレーが稼働していても、BPノードが停止していれば同様に生成不可となります。  
...
## Q2. 割り当てられたブロック生成スケジュールをSNSで公開してもいいですか？ [#sec-2]
<Callout type="info" title="A. NGです">
