...
すべてのコマンドは `$WORKING_DIR/keys` ディレクトリで実行します。
...
- `<YOUR_PUBLIC_IP>`：**リレーノードのパブリックIPアドレス**
...
${HOME}/.nix-profile/bin/cardano-node run +RTS -N --disable-delayed-os-memory-return -I0.1 -Iw300 -A16m -F1.5 -H2500M -RTS --topology \${TOPOLOGY} --database-path \${DB_PATH} --socket-path \${SOCKET_PATH} --host-addr \${HOSTADDR} --port \${PORT} --config \${CONFIG} --shelley-kes-key \${KES} --shelley-vrf-key \${VRF} --shelley-operational-certificate \${CERT}
EOF
